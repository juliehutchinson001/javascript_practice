...
JavaScript's global environment is full of controls your code can use to make things happen. These controls come in the form of JavaScript objects and functions. Global letiables are actually part of the __` window object `__.

>__Command + L__: Clearing the console in google developer tools.

__` alert('I made the browser message me'); `__
...
__` document.getElementById() `Method;__
...
>* Return the backgroundColor property __` object.style.backgroundColor; `__.
>* Set the backgroundColor property __` object.style.backgroundColor = "color"; `__.

__` document.getElementById('h2').style.color; `__
>* The color property, when __` object.style.color = "color"; `__ sets the color of the text.
>* when __` object.style.color; `__ returns the color representend in a string.
...
Changing the headline color in response to clicking a button. Selection is a way to identify an element for a browser, so the browser can find it and make it available for us to do something with it using JavaScript. Remember ___Document_ is a global letiable representing a webpage__. So now, a button is added to the html document, as well as an input element (so we can input text to affect the document):
...
>There is a letiable named button in app.js. Set its value to contain a reference to the button element in index.html with the ID of sayPhrase.
>
...
Select an element that doesn't have an ID, or select multiple elements on a page, using getElementsByTagName. 
...
>` let el = els[0]; `
>` el.style.color = 'blue' ` 
to change the index 0 of collection of paragraphs to blue color.

__` Document.getElementsByTagName(); `__
...
>* The item() method returns a numbered element from an HTMLCollection. In JS, it is easier to treat the collection as an array and to index it using array notation.
...
>` els[2].style.color = 'purple'; ` (lavender purple now)
...
> ` let paragraph = document.querySelector('.remove_me'); `
> ` let parent = removeMe.parentNode; `
> ` parent.removeChild(removeMe); `

1. First you need to fetch the element to be removed.
2. Then you need to call 
2. You get the paragraph's parentNode and assign it to a letiable parent.
...
Now, below in the event handler, the function can be called
for the new items:
...
To store the buttons on the existent list, the children
property can be used for the list and it can be looped through
each of the values of the collection:
...
__` ParentNode.firstElementChild `__
...
__` ParentNode.lastElementChild `__
>* It gives you the last DOM element child of a selected element.
>` const lastListItem = listUl.lastElementChild; `
>` lastListItem.style.backgroundColor = 'lightSkyBlue'; `
...
>* It returns the subsequent element of the selected element.

A delegated click event listener has been attached to the selected ul element, which is stored in the variable list. The handler is targeting each button in the list. 
...
>>`  let node = document.createTextNode("Some n text"); `
...
>* This allows us to create a timer and call a function to change properties repeatedly at defined intervals (in milliseconds).
...
__So, when to use .on or click?__

It is preferred .on over .click because the former can use less memory and work for dynamically added elements.
...
If you add a handler with `.on("click", handler)` you normally remove it with `.off("click", handler)` which will remove that very handler. 

Obviously this works only if you have a reference to the function, so what if you don't ? You use namespaces:
